...
try {
  ...
  const [resultOfCountryWithPopulationGreat8Million] = await connection.query(
    countryWithPopulationGreat8Million
  );
  console.log(
    "resultOfCountryWithPopulationGreat8Million:",
    resultOfCountryWithPopulationGreat8Million
  );

  const [resultOfCountryWithLandInName] = await connection.query(
    countryWithLandInName
  );
  ...
  console.log(
    "resultOfCityWithPopulationBetween500ToMillion",
    resultOfCityWithPopulationBetween500ToMillion
  );

  const [resultOfCountryInEUrope] = await connection.query(countryInEUrope);
  console.log("resultOfCountryInEUrope", resultOfCountryInEUrope);

  const [resultOfCountryDescendingOrderSurfaceArea] = await connection.query(
    countryDescendingOrderSurfaceArea
  );
  console.log(
  ...
  const [resultOfCityInNetherlands] = await connection.query(cityInNetherlands);
  console.log("resultOfCityInNetherlands", resultOfCityInNetherlands);

  const [resultOfRotterdamPopulation] = await connection.query(
    rotterdamPopulation
  );
  console.log("resultOfRotterdamPopulation", resultOfRotterdamPopulation);

  const [resultOfCountrySurfaceAreaTop10] = await connection.query(
    countrySurfaceAreaTop10
  );
  console.log(
    "resultOfCountrySurfaceAreaTop10",
    resultOfCountrySurfaceAreaTop10
  );

  const [resultOfCityPopulationTop10] = await connection.query(
    cityPopulationTop10
  );
  console.log("resultOfCityPopulationTop10", resultOfCityPopulationTop10);

  const [resultOfPopulationNumberOfTheWorld] = await connection.query(
    populationNumberOfTheWorld
  );
  console.log(
    "resultOfPopulationNumberOfTheWorld",
    resultOfPopulationNumberOfTheWorld
  );
